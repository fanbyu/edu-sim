...
class OasisAgentProfile:
    """OASIS Agent Profile data structure (Extended for Edu-Sim)"""
    # Common fields
    user_id: int
    user_name: str
    name: str
    ...
    agent_type: str = "student"  # student, teacher, class
    cognitive_level: float = 0.0  # IRT theta value
    anxiety_threshold: float = 0.5
    learning_style: str = "visual"  # visual, auditory, kinesthetic
    teaching_style: str = "heuristic"  # heuristic, direct (for teachers)
    ...
    def to_twitter_format(self) -> Dict[str, Any]:
        ...
        profile["agent_type"] = self.agent_type
        profile["cognitive_level"] = self.cognitive_level
        profile["anxiety_threshold"] = self.anxiety_threshold
        profile["learning_style"] = self.learning_style
        profile["teaching_style"] = self.teaching_style
...
class OasisProfileGenerator:
    ...
    def _build_individual_persona_prompt(
        ...
        """Build the detailed persona prompt for an individual entity (Education Context)"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        # Determine agent type based on entity type
        agent_type = "student" if "student" in entity_type.lower() else "teacher"
        
        return f"""Generate a detailed educational agent persona for this entity.
...
1. bio: Brief description of the agent's role in the educational system (MAX 50 words).
2. persona: Concise persona description (MAX 150 words), must cover:
   - For Students: Cognitive level, learning habits, anxiety threshold, preferred learning style.
   - For Teachers: Teaching style (heuristic vs direct), experience level, feedback quality.
3. age: Age as a number (must be an integer)
4. gender: Gender, must be in English: "male" or "female"
5. cognitive_level: A float value between -3 and 3 representing ability (IRT theta). Default to 0.0 if unknown.
6. anxiety_threshold: A float value between 0 and 1. Higher values mean more prone to exam anxiety.
7. learning_style: One of ["visual", "auditory", "kinesthetic", "reading_writing"].
8. teaching_style: (For teachers only) One of ["heuristic", "direct", "facilitator"].
9. mbti: MBTI type (e.g., INTJ, ENFP, etc.)

IMPORTANT: All string values must not contain unescaped newline characters. Use English."""
